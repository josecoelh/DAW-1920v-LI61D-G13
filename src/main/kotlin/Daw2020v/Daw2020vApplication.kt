...
class ApiConfig : WebMvcConfigurer {
	...
	override fun addInterceptors(registry: InterceptorRegistry) {
		registry.addInterceptor(interceptor).excludePathPatterns(listOf("$HOME/login", "$HOME/register" ))
		registry.addInterceptor(object : HandlerInterceptor{
			override fun preHandle(request: HttpServletRequest, response: HttpServletResponse, handler: Any): Boolean {
				if(request.session.getAttribute(USER_SESSION)!=null) response.sendRedirect(HOME)
				return true
			}
		}).addPathPatterns(listOf("$HOME/login", "$HOME/register" ))
